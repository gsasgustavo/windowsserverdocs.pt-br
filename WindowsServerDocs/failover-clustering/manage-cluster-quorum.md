...
As subseções a seguir fornecerão mais informações sobre as definições de configuração de quorum avançadas.
...
| Testemunha da nuvem     |  <ul><li>Um arquivo testemunha armazenado no armazenamento de BLOBs do Azure</li><li> Recomendado quando todos os servidores no cluster têm uma conexão de Internet confiável.</li>      |  Consulte [implantar uma testemunha em nuvem](./deploy-cloud-witness.md).       |

### <a name="node-vote-assignment"></a>Atribuição de voto de nó

Como opção de configuração de quorum avançado, você pode optar por atribuir ou remover votos de quorum por nó. Por padrão, todos os nós são votos atribuídos. Independentemente da atribuição de voto, todos os nós continuam a funcionar no cluster, receber atualizações do banco de dados do cluster e podem hospedar aplicativos.
