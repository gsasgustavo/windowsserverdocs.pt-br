...
3. Compartilhe os dados no servidor de origem e torne-os acessíveis por meio de um namespace do DFS. Isso é importante para garantir que os usuários possam ainda acessá-lo se o nome do servidor for alterado em um local de desastre.
   a.  Você pode criar compartilhamentos correspondentes no servidor de destino, que não estará disponível durante as operações normais, b.  Não adicione o servidor de destino ao namespace do DFS Namespaces ou, se você fizer isso, certifique-se de que todos os seus destinos de pasta estejam desabilitados.
