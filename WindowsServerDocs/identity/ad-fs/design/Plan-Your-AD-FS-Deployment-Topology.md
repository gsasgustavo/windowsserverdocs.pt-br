...
|Recursos de AD FS|Detecção de reprodução do token SAML/Web Services Federation|Não|Sim
|Recursos de banco de dados|A redundância básica do banco de dados usa a replicação pull, em que um ou mais servidores hospedam uma cópia somente leitura das alterações de solicitação do banco de dados que são feitas em um servidor de origem que hospeda uma cópia de leitura/gravação do banco de dados|Sim|Não
|Recursos de banco de dados|Redundância de banco de dados usando soluções de alta disponibilidade, como clustering de failover ou espelhamento (somente na camada do banco de dados) **Observação:** todas as topologias de implantação do AD FS dão suporte ao clustering na camada de serviço do AD FS.|Não|Sim
