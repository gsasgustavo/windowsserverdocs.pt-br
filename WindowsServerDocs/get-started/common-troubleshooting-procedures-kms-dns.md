...
1. Se o problema persistir, execute os seguintes comandos:
   ...
   ```
   > [!NOTE]
   > Nesse comando, \<IP Address\> representa o endereço IP do computador host KMS e \<port\> representa a porta TCP usada pelo KMS.
...
Por padrão, um host KMS registra seus registros SRV na zona DNS que corresponde ao domínio do computador host do KMS. Por exemplo, suponha que um host KMS ingresse no domínio contoso.com. Nesse cenário, o host KMS registra o registro SRV `_vlmcs` na zona DNS contoso.com. Portanto, o registro identifica o serviço como `_VLMCS._TCP.CONTOSO.COM`.
